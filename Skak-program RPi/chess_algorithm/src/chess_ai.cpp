		...
		for (char y = 0; y < BOARD_HEIGHT; y++) {
			for (char x = 0; x < BOARD_WIDTH; x++) {
				...
				for (int i = 0; i < all_moves.size(); i++) {
					...
					if (new_edge->dest.get_material(Chess_Board::Piece_color::WHITE) < - 1000 || new_edge->dest.get_material(Chess_Board::Piece_color::BLACK) < -1000) {
...
float Chess_AI::get_children_average_material_difference(Node * node, int depth) {
	...
	if (num_children == 0) {
		return m_defense * (float) node->get_material(m_color) - m_offense * (float) node->get_material(other_color(m_color));
	}
